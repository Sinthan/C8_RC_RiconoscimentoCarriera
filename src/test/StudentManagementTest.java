...
class StudentManagementTest extends Mockito {
  ...
  public void returnRequestStatusOK2() throws ServletException, IOException {
    ...
    when(sessione.getAttribute("flag")).thenReturn(0);
    when(sessione.getAttribute("user")).thenReturn(s);
    when(request.getRequestDispatcher("/WEB-INF/GUIStudentRC/viewRCRequestStatus.jsp"))
        .thenReturn(dsp);
    sm.doGet(request, response);
  ...
  public void returnUniversityList() throws ServletException, IOException {
    ...
    when(request.getRequestDispatcher("/WEB-INF/GUIStudentRC/createRCRequest1.jsp"))
        .thenReturn(dsp);
    sm.doGet(request, response);
    verify(dsp).forward(request, response);
  }
  ...
  public void createRequestRC1Test2() throws ServletException, IOException {
    ...
    when(request.getParameter("universita"))
        .thenReturn("Università degli Studi di NAPOLI Federico II");
  ...
  public void createRequestRC1Test4() throws ServletException, IOException {
    ...
    when(request.getRequestDispatcher("/WEB-INF/GUIStudentRC/createRCRequest2.jsp"))
        .thenReturn(dsp);
    sm.doGet(request, response);
    verify(dsp).forward(request, response);
  ...
  public void createRequestRC2InvalidExamName() throws ServletException, IOException {
    ...
    // TODO
    when(request.getParameter("examName" + currentExamRow)).thenReturn("||AnalisiI*Â£#");
  ...
  public void createRequestRC2Ok() throws ServletException, IOException {
    ...
    currentExamRow = 2;
    when(request.getParameter("examName" + currentExamRow))
        .thenReturn("Algoritmi e strutture dati I");
    when(request.getParameter("CFU" + currentExamRow)).thenReturn("9");
    when(request.getParameter("programLink" + currentExamRow)).thenReturn(
        "http://cs-informatica.dieti.unina.it/index.php/it/corsi-di-laurea/insegnamenti/laurea-triennale/15-corsi-di-laurea/corsi/148-algoritmi-e-strutture-dati-i");
    currentExamRow = 3;
    ...
    when(request.getParameter("programLink" + currentExamRow))
        .thenReturn("www.sito.it/dati/esame/prog1");
    currentExamRow = 4;
    ...
    FilePDF file =
        new FilePDF("/WebContent" + pdfSaveFolder + "/" + s.getEmail() + "/" + "fake.pdf", reqRCID);
    ...
    when(request.getRequestDispatcher("/EnglishValidation/StudentRCRequestRedirector?flag=1"))
        .thenReturn(dsp);
    sm.doGet(request, response);
  }
