...
class RequestRCManagementTest extends Mockito {
  ...
  /*
   * @Test public void returnRequestOK() { UCDAO ucDAO = new UCDAO(); String email = "uc@unisa.it";
   * String password = "password"; UC userRC = ucDAO.doRetrieveUc(email, password);
   * request.getSession().setAttribute("userRC",userRC); int requestRCID = 1; RequestRC rRC =
   * requestRCDAOStub(requestRCID); assertNotNull(rRC); }
   * 
   * @Test public void returnRequestFail() { UCDAO ucDAO = new UCDAO(); String email =
   * "uc@unisa.it"; String password = "password"; UC userRC = ucDAO.doRetrieveUc(email, password);
   * request.getSession().setAttribute("userRC",userRC); int requestRCID = 567890; RequestRC rRC =
   * requestRCDAOStub(requestRCID); assertNull(rRC); }
   * 
   * 
   * public RequestRC requestRCDAOStub(int requestRCID) { Connection conn = (Connection) new
   * DbConnection().getInstance().getConn();
   * 
   * try { PreparedStatement ps = conn.prepareStatement( " SELECT  * FROM request_rc " +
   * "WHERE id_request = ?"); ps.setInt(1, requestRCID); ResultSet rs = ps.executeQuery(); if
  ...
  public void setUpSystem() {
    ...
    System.setProperty("catalina.base", "C:\\Users\\" + user
        + "\\ISworkspace\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0");
  }
