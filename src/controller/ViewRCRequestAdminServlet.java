    ...
    if (req != null) {
      ...
      if (!filesPDF.isEmpty()) {
        ...
        for (int i = 0; i < filesPDF.size(); i++) {
          ...
          String relativePath = filesPDF.get(i).getPDFLink().substring(indexWC + 1,
              filesPDF.get(i).getPDFLink().length());
          if (indexWC > -1) {
            ...
            goBackWithError("Path del PDF non valido, impossibile mostrare il PDF.", request,
                response);
            return;
          }
        }
        ...
        if (s != null) {
          ...
          if (!examList.isEmpty()) {
            ...
            goBackWithError(
                "Impossibile caricare la pagina, esami non trovati, si prega di riprovare.",
                request, response);
          }
        } else {
          goBackWithError(
              "Impossibile caricare la pagina, dati dello studente non trovati, "
              + "si prega di riprovare.",
              request, response);
        }
      } else {
        goBackWithError(
            "Impossibile caricare la pagina, PDF del certificato non trovato, "
            + "si prega di riprovare.",
            request, response);
      }
    } else {
