...
public class StudentManagement extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (flag == 0) {
      ...
      if (UniSel.equals("defaultUni")) {
        ...
        request.setAttribute("errorCR1",
            "Formato file non accettato, inserire file in formato PDF.");
        dis = request.getRequestDispatcher("/WEB-INF/GUIStudentRC/createRCRequest1.jsp");
        dis.forward(request, response);
        ...
        if (SAVE_DIR2.equals("") || SAVE_DIR2.equals(null)) {
          ...
          dis = request.getServletContext()
              .getRequestDispatcher("/WEB-INF/GUIStudentRC/createRCRequest1.jsp");
          dis.forward(request, response);
        }
        ...
        try {
          filePart1.write(
              SAVE_DIR2 + "/WebContent" + pdfSaveFolder + "/" + s.getEmail() + "/" + "ID.pdf");
        } catch (Exception msg) {
        ...
        try {
          filePart2.write(
              SAVE_DIR2 + "/WebContent" + pdfSaveFolder + "/" + s.getEmail() + "/" + "CP.pdf");
        } catch (Exception msg) {
          System.out.println(msg.getLocalizedMessage());
        }
      ...
      if (request.getSession().getAttribute("newRequestRC") == null
        ...
        request.setAttribute("errorCR1",
            "Alcuni dati sono andati persi, si prega di ricompilare la richiesta.");
        dis = request.getRequestDispatcher("/WEB-INF/GUIStudentRC/createRCRequest1.jsp");
        dis.forward(request, response);
        return;
      }
      ...
      for (int currentExamRow = 1; currentExamRow <= examsInserted; currentExamRow++) {
        ...
        if (!name.matches("^(\\w+\\s?\\-?)*(\\-?\\s*\\w*)*$")) {
          request.setAttribute("errorCR2", "Il nome dell&#8217;esame alla riga " + currentExamRow
              ...
              + " pi&#249; i caratteri \"-\" e \"_\". La lunghezza deve essere "
              + "compresa tra i 2 e i 50 caratteri");
          dis = request.getRequestDispatcher("/WEB-INF/GUIStudentRC/createRCRequest2.jsp");
          dis.forward(request, response);
          return;
        } else if (CFU < 0 || CFU > 31) {
          request.setAttribute("errorCR2",
              "I CFU dell&#8217;esame alla riga " + currentExamRow
          ...
          return;
            ...
            "^(?:(http(s)?|ftp):\\/\\/)?([-a-zA-Z0-9@:%._\\\\+~#=]+"
            + "(\\.[a-zA-Z0-9]{1,6})+)(\\/([-a-zA-Z0-9()@:%_\\\\+.~#?&=]*))*")) {
          request.setAttribute("errorCR2", "Il link dell&#8217;esame alla riga " + currentExamRow
              + " non rispetta un formato valido oppure la "
              + "lunghezza non &#232; compresa tra i 4 e i 256 caratteri");
          dis = request.getRequestDispatcher("/WEB-INF/GUIStudentRC/createRCRequest2.jsp");
          dis.forward(request, response);
          return;
        }
      ...
      if (insertRequestRCResult > 0 && insertFile1Result > 0 && insertFile2Result > 0) {
        for (int result : insertExamResults) {
          if (result == -2 || result == -4) {
            System.out.println(
                "Si sono verificati errori sull'inserimento degli esami all'interno del database.");
            goBackWithError(reqRCID,
                "Si &#232; verificato un errore durante il salvataggio"
                + " della richiesta. Si prega di riprovare.",
                request, response);
            return;
          }
        }
        for (int result : insertContainsRelationResults) {
          if (result == 0 || result == -2) {
            System.out.println(
                "Si sono verificati errori sull'inserimento delle"
                + " relazioni Contains all'interno del database.");
            goBackWithError(reqRCID,
                "Si &#232; verificato un errore durante il salvataggio "
                + "della richiesta. Si prega di riprovare.",
                request, response);
        ...
        System.out.println(
            "Si sono verificati errori sull'inserimento della richiesta"
            + " o dei file pdf all'interno del database.");
        goBackWithError(reqRCID,
            "Si &#232; verificato un errore durante il salvataggio della "
            + "richiesta. Si prega di riprovare.",
            request, response);
        return;
      }
  ...
  private void goBackWithError(int requestRCID, String message, HttpServletRequest request,
