...
public class ReportManagementServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    String stName = sDao.doRetrieveStudentByEmail(studentMail).getName() + " "
        + sDao.doRetrieveStudentByEmail(studentMail).getSurname();
    String messageBody = "Gentile " + stName + ";\n"
        + "In allegato trover&#224; il PDF contente l'esito della sua richiesta di"
    ...
    if (req != null) {
      ...
      for (int i = 1; i < rows; i++) {
        ...
        if (mode.length() > 5000) {
          goBackWithError("Impossibile salvare la bozza, la modalita di convalida dell'esame "
              + examName + " supera i 5000 caratteri", request, response);
          return;
        }
      ...
      if (note.length() > 5000) {
        goBackWithError("Impossibile salvare la bozza, la nota supera i 5000 caratteri", request,
            response);
        return;
      }
      ...
      File fileSuggOW = new File(projectPath + "/" + "WebContent" + pdfSaveFolder + "/"
          + s.getEmail() + "/" + "suggOverwrite.txt");
      fileSuggOW.delete();
      fileSuggOW = new File(projectPath + "/" + "WebContent" + pdfSaveFolder + "/" + s.getEmail()
          + "/" + "suggOverwrite.txt");
      ...
      if (request.getParameter("closeRCRequestBtn") != null) {
        ...
        for (int i = 1; i < rows; i++) {
          ...
          if (suggDao.doRetrieveSuggestionByName(universityName, examName2, externalCFU) == null) {
            ...
            sugg = new Suggestion(req.getUniversityID(), examList.get(i - 1).getExamName(),
                CFUExt.get(i - 1), ValidatedCFUArray.get(i - 1),
                examList.get(i - 1).getValidationProcedure(), date);
        ...
        for (int i = 0; i < suggOverwrite.size(); i++) {
          ...
          if (suggOverwrite.get(i) != null) {
            ...
            sugg.setExternalStudentCFU(
                Integer.parseInt(request.getParameter("externalExamCFU" + (i + 1))));
            sugg.setValidatedCFU(examList.get(i).getValidatedCFU());
            sugg.setValidationMode(examList.get(i).getValidationProcedure());
            Suggestion suggestion = suggDao.doRetrieveSuggestionByName(req.getUniversityID(),
                sugg.getExamName(), sugg.getExternalStudentCFU());
        ...
        RequestDispatcher requestDispatcher =
            request.getRequestDispatcher("/ViewReportAdminServlet");
        requestDispatcher.forward(request, response);
      }
    } else {
      // if RCRequest = null
      goBackWithError(
          "Impossibile caricare la pagina, errore nel recupero della richiesta "
          + "selezionata, si prega di riprovare.",
          request, response);
      return;
    }
  }
  ...
  /**
   * @see go back to page with status error.
   */
  public void goBackWithError(String message, HttpServletRequest request,
