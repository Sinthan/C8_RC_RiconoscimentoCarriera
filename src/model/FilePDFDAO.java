...
/**
 * FilePDFDAO interacts with the database in order to manage files related to a previous career
 * recognition request.
 */
...
public class FilePDFDAO implements FilePDFDAOInterface {
  ...
  public int insertFilePDF(FilePDF file) {
    if (file.getPDFLink().equals("") || file.getRequestRCID() == -1) { 
      // Checks if attributes are set
      System.out.println(
          "Please set the PDF link and the RequestRC ID before trying to add the "
          + "FilePDF to the database.");
      return -2;
    }
    ...
    /*
     * Adds the 2 given parametric values in the FILE_PDF table. The file ID is automatically
     * generated from the database, as it is defined as an auto increment value, so it's not
     * required to create a new one here.
     */
    String insertSQL =
        "INSERT INTO FILE_PDF " + " (LINK_PDF, FK_ID_REQUEST_RC) " + " VALUES (?, ?)";
    try {
      ...
      connection.commit();
      System.out.println("insertFilePDF(result=" + result + ": " + file.toString()); 
  ...
  /**
   * Retrieves all the <tt>FilePDF</tt> objects related to a specific <tt>RequestRC</tt> (identified
   * through an ID).
   * 
   * @param requestRCID the <tt>RequestRC</tt> ID number that the <tt>RequestRC</tt> object must
   *        match
   * @return an <tt>ArrayList</tt> containing the <tt>FilePDF</tt> objects that match the given
   *         <tt>RequestRC</tt> ID
   * @see FilePDF
   * @author Gianluca Rossi
   */
  ...
  public ArrayList<FilePDF> doRetrieveAllFilePDFByIDRequestRC(int requestRCID) {
    ...
    // Selects the IDs and the link of the FilePDF records that are related to the specified
    // RequestRC ID
    String selectSQL =
        "SELECT  F.ID_PDF, F.LINK_PDF " + "FROM Request_RC R " + "	INNER JOIN FILE_PDF F\n"
    ...
    try {
      ...
      System.out
          .println("doRetrieveAllFilePDFByIDRequestRC: error while executing the query\n" + e);
