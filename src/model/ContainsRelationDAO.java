...
/**
 * <tt>ContainsRelationDAO</tt> Implementation of the ContainsRelationDAOInterface, manages and
 * retrieves data related to <tt>ContainsRelation</tt> objects from the MySQL database.
 * 
...
public class ContainsRelationDAO implements ContainsRelationDAOInterface {
  ...
  public int insertContainsRelation(ContainsRelation conRel) {
    if (conRel.getExamID() == -1 || conRel.getRequestRCID() == -1) { // Checks if attributes are set
      System.out.println(
          "Please set the Exam ID and the RequestRC ID before trying to add"
          + " the ContainsRelation to the database.");
      return -2;
    }
    ...
    String insertSQL = "INSERT INTO CONTAINS " + " (FK_REQUEST_RC, FK_EXAM) " + " VALUES (?, ?)";
    ...
    try {
      ...
      System.out.println("insertContainsRelation(result=" + result + ": " + conRel.toString()); 
  ...
  public ArrayList<ContainsRelation> doRetrieveAllContainsRelationByIDExam(int idExam) {
    ...
    try {
      ...
      System.out
          .println("doRetrieveAllContainsRelationByIDExam: error while executing the query\n" + e);
