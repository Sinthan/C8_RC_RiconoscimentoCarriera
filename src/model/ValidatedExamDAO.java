...
public class ValidatedExamDAO implements ValidatedExamDAOInterface {
   ...
   * @return
   *         <ul>
   *         <li>a positive count if the insertion succeeded
  ...
  public int insertValidatedExam(ValidatedExam vExam) {
    if (vExam.getExamName().equals("")) { // Checks if attributes are set
      System.out
          .println("Please set the Exam's name and cfu before trying to add it to the database.");
      return -2;
    }
    Connection connection = null;
    PreparedStatement preparedStatement = null;
    String insertSQL = "INSERT INTO VALIDATE_EXAM"
        + " (NAME_EXAM, CFU_CONVALIDATED, MODE_VALIDATION,FK_ID_REPORT) " + " VALUES (?, ?, ?, ?)";
    int result = 0;
    ...
    try {
      ...
      new RuntimeException(
          "Couldn't insert the exam \"" + vExam.getExamName() + "\" in the database " + e);
    } finally {
      // Statement release
    ...
    System.out.println("insertExam(result=" + result + ": " + vExam.toString()); 
    return 0;
  }
   ...
   * @return
   *         <ul>
   *         <li>a positive count if the update succeeded
   *         <li>0 if nothing was altered into the database
   *         <li>-1 if the update succeeded, but the database didn't return any information about
  ...
  public int updateValidatedExam(ValidatedExam vExam) {
    ...
    if (vExam == null) {
      ...
      try {
        ...
        new RuntimeException(
            "Couldn't find the exam \"" + vExam.getExamName() + "\" in the database " + e);
      } finally {
        // Statement release
        if (preparedStatement != null)
          try {
  ...
  /**
   * Retrieves a <tt>ValidatedExam</tt> object that matches the given <tt>Report</tt> (identified
   * through an ID), and the given exam name.
   * 
   * @param reportID the report ID number that the <tt>ValidatedExam</tt> object must have
   * @param examName the exam name (used in the external university) that the <tt>ValidatedExam</tt>
   *        object must have
  ...
  public ValidatedExam doRetrieveValidatedExam(int reportID, String examName) {
    if (reportID < 0 || examName.equals("")) {
      ...
      // Selects the exams that match the 2 given parametric values
      String selectSQL =
          "SELECT * FROM VALIDATE_EXAM " + " WHERE FK_ID_REPORT = ? AND NAME_EXAM = ?";
      try {
        ...
        new RuntimeException(
            "Couldn't find the exam \"" + exam.getExamName() + "\" in the database " + e);
      } finally {
        // Statement release
        if (preparedStatement != null)
          try {
   ...
   * @return an <tt>ArrayList</tt> containing the <tt>ValidatedExams</tt> objects that match the
   *         given report ID
   * @see Report
   */
  @Override
  public ArrayList<ValidatedExam> doRetrieveValidatedExamsByReportID(int reportID) {
    ...
    String selectSQL = "SELECT * FROM VALIDATE_EXAM " + " WHERE FK_ID_REPORT = ?";
    try {
      ...
      System.out
          .println("doRetrieveValidatedExamsByReportID: error while executing the query\n" + e);
  ...
  /*
   * @Override public int deleteValidatedExam(ValidatedExam vExam) { if (vExam.getVExamID()< 0) { //
   * Checks if parameter is a valid ID
   * System.out.println("deleteValidatedExam: Please enter a valid ExamID."); return -1; }
   * 
   * Connection connection = null; PreparedStatement preparedStatement = null; int result = 0;
   * 
   * String deleteSQL = "DELETE * FROM VALIDATE_EXAM WHERE ID_EXAM_VALIDATE = ?"; try { connection =
   * DbConnection.getInstance().getConn(); preparedStatement =
   * connection.prepareStatement(deleteSQL); // Setting parameter preparedStatement.setInt(1,
   * vExam.getVExamID()); //cheking if exam exist
   * if(doRetrieveValidatedExam(vExam.getVExamID(),vExam.getExamName())==null) {
   * System.out.println("Exam don't exist already"); return -2; } // Executing the deletion
   * 
   * result = preparedStatement.executeUpdate(); connection.commit(); return 0; } catch(SQLException
   * e) { System.out.println("deleteValidatedExam: error while executing the query\n" + e); new
   * RuntimeException("Couldn't delete the Exam " + e); } finally { // Statement release
