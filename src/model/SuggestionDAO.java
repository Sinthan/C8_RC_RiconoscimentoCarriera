...
public class SuggestionDAO implements SuggestionDAOInterface {
  ...
  public int insertSuggestion(Suggestion suggestion) {
    ...
    String insertSQL = "INSERT INTO SUGGESTION "
        + " (NAME_UNIVERSITY, NAME_EXAM_EXTERN, NUMBER_CFU_EXTERN, VALIDATED_CFU, "
        + "MODE_VALIDATION, DATE_VALIDATION) "
        + " VALUES (?, ?, ?, ?, ?, ?)";
    ...
    System.out.println("insertSuggestion(result=" + result + ": " + suggestion.toString()); 
   ...
   * 
   * @param univerityName the <tt>University</tt> name that the <tt>Suggestion</tt> object must
   *        match
  ...
  public Suggestion doRetrieveSuggestionByName(String universityName, String examName) {
    if (universityName.equals("") || examName.equals("")) { // Checks if parameter is a valid ID
      System.out.println(
          "doRetrieveSuggestionByName: Please enter a valid name "
          + "for the university and/or the exam.");
      return null;
    }
    ...
    String selectSQL =
        "SELECT * FROM SUGGESTION " + " WHERE NAME_UNIVERSITY = ? AND NAME_EXAM_EXTERN = ?";
    try {
      connection = DbConnection.getInstance().getConn();
      preparedStatement = connection.prepareStatement(selectSQL);
  ...
  public Suggestion doRetrieveSuggestionByName(String universityName, String examName, int CFU) {
    if (universityName.equals("") || examName.equals("") || CFU == 0) { 
   ...
   * @return
   *         <ul>
   *         <li>0 if the update succeeded
   *         <li>-1 if input param is null
   *         <li>-2 if param doesen't exists in database
   * @author Lorenzo Maturo
   */
  /*
   * @Override public int updateSuggestion(Suggestion sugg) {
   * 
   * if(sugg==null) { System.out.println( "updateSuggestion: Void sugg passed" ); return -1; } else
   * if( doRetrieveSuggestionByName(sugg.getUniversityName(), sugg.getExamName(),
   * sugg.getExternalStudentCFU() )==null){ System.out.println(
   * "updateSuggestion: sugg doesn't exists in DB" ); return -2; }else {
   * 
   * Connection connection = null; PreparedStatement preparedStatement = null;
   * 
   * // Selects the sugg String updateSQL =
   * "UPDATE suggestion SET VALIDATED_CFU = ?, MODE_VALIDATION = ?, DATE_VALIDATION = ?" +
   * " WHERE ( NAME_UNIVERSITY = ? AND NAME_EXAM_EXTERN = ? AND NUMBER_CFU_EXTERN = ? ); "; try {
   * 
   * connection = DbConnection.getInstance().getConn(); preparedStatement =
   * connection.prepareStatement(updateSQL);
   * 
   * //Set Data Calendar calendar = Calendar.getInstance(); java.util.Date currentDate =
   * calendar.getTime(); java.sql.Date sqlDate = new java.sql.Date(currentDate.getTime());
   * 
   * // Setting parameters preparedStatement.setInt(1, sugg.getValidatedCFU());
   * preparedStatement.setString(2, sugg.getValidationMode()); preparedStatement.setDate(3,
   * sqlDate); preparedStatement.setString(4, sugg.getUniversityName());
   * preparedStatement.setString(5, sugg.getExamName()); preparedStatement.setInt(6,
   * sugg.getExternalStudentCFU()); preparedStatement.executeUpdate(); connection.commit();
   * 
   * 
   * } catch(SQLException e) {
   * System.out.println("updateSuggestion: error while executing the query\n" + e); new
   * RuntimeException("Couldn't find the sugg \"" + sugg.getExamName() + "\" in the database " + e);
   * } finally { // Statement release if(preparedStatement != null) try { preparedStatement.close();
