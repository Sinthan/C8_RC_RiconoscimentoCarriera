...
/**
 * ReportDAO. interacts with the database in order to manage the report data for a prefressa career
 * recognition request
 */
...
public class ReportDAO implements ReportDAOInterface {
   ...
   */
  /*
   * @Override public int insertReport(Report report) { if (report.getValidatedExamsList()==null) {
   * // Checks if attributes are set System.out.println("Void report"); return -2; }
   * 
   * Connection connection = null; PreparedStatement preparedStatement = null; int result = 0;
   * 
   * String insertSQL = "INSERT INTO REPORT " + " (NOTE) " + " VALUES (?)"; try { connection =
   * DbConnection.getInstance().getConn(); preparedStatement =
   * connection.prepareStatement(insertSQL); // Setting parameters preparedStatement.setString(1,
   * report.getNote()); // Executing the insertion result = preparedStatement.executeUpdate();
   * connection.commit(); System.out.println("insertRequestRC(result=" + result + ": " +
   * report.toString()); // Logging the operation } catch(SQLException e) {
   * System.out.println("insertReport: error while executing the query\n" + e); new
   * RuntimeException("Couldn't insert the RequestRC " + e); } finally { // Statement release
   ...
   */
  /*
   * @Override public int updateReport(Report report) {
   * 
   * if(report==null) {
   * 
   * System.out.println("Invalid report passed"); return-1;
   * 
   * } else if(doRetrieveReportByReportID(report.getReportID())==null) {
   * 
   * System.out.println("report doesn't exists in DB"); return -2; } else {
   * 
   ...
   */
  /*
   * @Override public int updateValidatedExamsList(int reportID, ArrayList<ValidatedExam>
   * validatedExamsList) { // TODO Auto-generated method stub return 0; }
   */
  ...
  public int updateNote(int reportID, String note) {
    if (reportID < 0) {
      ...
      String updateSQL = "UPDATE REPORT SET NOTE = ?" + " WHERE ID_REPORT = ?; ";
      try {
  ...
  public Report doRetrieveReportByReportID(int reportID) {
    ...
    String selectSQL = "SELECT * FROM REPORT " + " WHERE ID_REPORT = ?";
    try {
      connection = DbConnection.getInstance().getConn();
   ...
   */
  /*
   * @Override public ArrayList<ValidatedExam> doRetrieveValidatedExamsByReportID(int reportID) {
   * ValidatedExamDAO vDao = new ValidatedExamDAO(); ArrayList<ValidatedExam> exams =
   * vDao.doRetrieveValidatedExamsByReportID(reportID); return exams; }
   */
   ...
   * @return
   *         <ul>
   *         <li>a positive count if the deletion succeeded
   ...
   *         <li>-2 if the passed parameter is not a valid <tt>Report</tt> ID
   *         </ul>
